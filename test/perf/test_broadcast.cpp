...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < num_warmup_iters; i++) {
    sdcclBroadcast(sendbuff, recvbuff, max_bytes / sizeof(float), DATATYPE, 0,
                    comm, stream);
  }
  ...
  for (int i = 0; i < num_warmup_iters; i++) {
    sdcclBroadcast(sendbuff, recvbuff, min_bytes / sizeof(float), DATATYPE, 0,
                    comm, stream);
  }
  devHandle->streamSynchronize(stream);
  ...
  for (size_t size = min_bytes; size <= max_bytes; size *= step_factor) {
    ...
    for (int r = begin_root; r <= end_root; r++) {
      ...
      for (int i = 0; i < num_iters; i++) {
        sdcclBroadcast(sendbuff, recvbuff, count, DATATYPE, r, comm, stream);
      }
      devHandle->streamSynchronize(stream);
