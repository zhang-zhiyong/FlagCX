...
sdcclResult_t getBusId(int cudaDev, int64_t *busId) {
  ...
  TRACE(SDCCL_INIT, "busId for cudaDev %d is %s", cudaDev, busIdStr);
  SDCCLCHECK(busIdToInt64(busIdStr, busId));
  return sdcclSuccess;
}
 ...
 *
 * This string can be overridden by using the SDCCL_HOSTID env var.
 */
...
uint64_t getHostHash(void) {
  ...
  if ((hostId = sdcclGetEnv("SDCCL_HOSTID")) != NULL) {
    INFO(SDCCL_ENV, "SDCCL_HOSTID set by environment to %s", hostId);
  ...
  TRACE(SDCCL_INIT, "unique hostname '%s'", hostHash);

  return getHash(hostHash, strlen(hostHash));
}

SDCCL_PARAM(TopoDetectionDisable, "TOPO_DETECTION_DISABLE", 0);
...
uint64_t getPidHash(void) {
  ...
  pname[plen + len] = '\0';
  TRACE(SDCCL_INIT, "unique PID '%s'", pname);
...
__thread struct sdcclThreadSignal sdcclThreadSignalLocalInstance =
    sdcclThreadSignalStaticInitializer();

void *sdcclMemoryStack::allocateSpilled(struct sdcclMemoryStack *me,
  ...
  { // If the next hunk we're going to allocate wouldn't be big enough but the
    ...
    mallocSize = nextSize;
    INFO(SDCCL_ALLOC, "%s:%d memory stack hunk malloc(%llu)", __FILE__,
...
unhunked : { // We need to allocate the object out-of-band and put an Unhunk
  ...
  INFO(SDCCL_ALLOC, "%s:%d memory stack non-hunk malloc(%llu)", __FILE__,
       __LINE__, (unsigned long long)mallocSize);
...
void sdcclMemoryStackDestruct(struct sdcclMemoryStack *me) {
  ...
  // the hunks.
  struct sdcclMemoryStack::Frame *f = &me->topFrame;
  while (f != nullptr) {
    struct sdcclMemoryStack::Unhunk *u = f->unhunks;
    while (u != nullptr) {
      free(u->obj);
      u = u->next;
    }
  ...
  // Free hunks
  struct sdcclMemoryStack::Hunk *h = me->stub.above;
  while (h != nullptr) {
    struct sdcclMemoryStack::Hunk *h1 = h->above;
    free(h);
    h = h1;
  }
}
...
const char *sdcclDatatypeToString(sdcclDataType_t type) {
  switch (type) {
    case sdcclInt8: // sdcclChar
      return "sdcclInt8";
    case sdcclInt32: // sdcclInt
      return "sdcclInt32";
    case sdcclUint32:
      return "sdcclUint32";
    case sdcclInt64:
    ...
    case sdcclUint64:
      return "sdcclUint64";
    case sdcclFloat16: // sdcclHalf
      return "sdcclFloat16";
    case sdcclFloat32: // sdcclFloat
      return "sdcclFloat32";
    case sdcclFloat64: // sdcclDouble
      return "sdcclFloat64";
#if defined(__CUDA_BF16_TYPES_EXIST__)
    case sdcclBfloat16:
      return "sdcclBfloat16";
#endif
    default:
      return "Unknown";
  }
...
const char *sdcclAlgoToString(int algo) {
  switch (algo) {
    case SDCCL_ALGO_TREE:
      return "TREE";
    case SDCCL_ALGO_RING:
      return "RING";
    case SDCCL_ALGO_COLLNET_DIRECT:
      return "COLLNET_DIRECT";
    case SDCCL_ALGO_COLLNET_CHAIN:
      return "COLLNET_CHAIN";
    case SDCCL_ALGO_NVLS:
      return "NVLS";
    case SDCCL_ALGO_NVLS_TREE:
      return "NVLS_TREE";
    default:
      return "Unknown";
  }
...
const char *sdcclProtoToString(int proto) {
  switch (proto) {
    case SDCCL_PROTO_LL:
      return "LL";
    case SDCCL_PROTO_LL128:
      return "LL128";
    case SDCCL_PROTO_SIMPLE:
      return "SIMPLE";
    default:
      return "Unknown";
  }
...
void *sdcclOpenLib(const char *path, int flags,
...
sdcclResult_t sdcclHomoCommInit(sdcclUniqueId_t commId,
  ...
  if (comm->homoRank == 0) {
    memcpy((void *)&uniqueIdData[rank], (void *)commId, sizeof(sdcclUniqueId));
  }
  SDCCLCHECK(
      bootstrapAllGather(state, (void *)uniqueIdData, sizeof(sdcclUniqueId)));
...
FlagScaleConfig readFlagScaleJson(const std::string &filename) {
  ...
  if (actual_filename.empty()) {
    const char *base = getenv("SDCCL_TUNE_FILE");
    if (base != nullptr && base[0] != '\0') {
      ...
      throw std::runtime_error(
          "SDCCL_TUNE_FILE environment variable is not set. "
          "Please set SDCCL_TUNE_FILE to specify the flagscale.json file "
          "path.");
    }
  }
  ...
  if (curTuneGroupIdx == -1) {
    throw std::runtime_error(
        "SDCCL_TUNE_GROUP_IDX environment variable is not set or invalid. "
        "Please set SDCCL_TUNE_GROUP_IDX to specify the tune group index.");
  }
