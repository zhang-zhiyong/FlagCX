...
struct shmHandleInternal {
  ...
  char *shmPath;
  char *shmPtr;
  void *devShmPtr; // not used in SDCCL
  size_t shmSize;
  size_t realShmSize;
  int *refcount;
};
...
sdcclResult_t sdcclShmOpen(char *shmPath, size_t shmPathSize, size_t shmSize,
  ...
  *handle = *shmPtr =
  ...
  if (create) {
     ...
     */
    if (shmPath[0] == '\0') {
      snprintf(shmPath, shmPathSize, "/dev/shm/sdccl-XXXXXX");
      fd = mkstemp(shmPath);
    } else {
    ...
    INFO(SDCCL_ALLOC, "Allocated %ld bytes of shared memory in %s",
         realShmSize, shmPath);
  } else {
  ...
  if (create) {
    *(int *)(hptr + shmSize) = refcount;
    ...
    int remref = sdcclAtomicRefCountDecrement((int *)(hptr + shmSize));
    if (remref == 0) {
...
fail:
  ...
  if (tmphandle) {
    shmHandleInit(fd, shmPath, shmSize, realShmSize, hptr, dptr, create,
                  tmphandle);
    sdcclShmClose((sdcclShmHandle_t)tmphandle);
    tmphandle = NULL;
  }
  hptr = NULL;
  dptr = NULL;
...
sdcclResult_t sdcclShmAllocateShareableBuffer(size_t size,
  ...
  desc->shmSize = size;
  SDCCLCHECK(sdcclShmOpen(shmPath, sizeof(shmPath), size, hptr, dptr, 1,
                            &desc->handle));
  memcpy(desc->shmSuffix, shmPath + sizeof("/dev/shm/sdccl-") - 1,
         sizeof(desc->shmSuffix));
  INFO(SDCCL_ALLOC, "MMAP allocated shareable host buffer %s size %zi ptr %p",
       shmPath, size, *hptr);
  return sdcclSuccess;
}
...
sdcclResult_t sdcclShmImportShareableBuffer(sdcclShmIpcDesc_t *desc,
  ...
  snprintf(shmPath, sizeof(shmPath), "/dev/shm/sdccl-%s", desc->shmSuffix);
  SDCCLCHECK(sdcclShmOpen(shmPath, sizeof(shmPath), desc->shmSize, hptr, dptr,
                            -1, &descOut->handle));
  INFO(SDCCL_ALLOC, "MMAP imported shareable host buffer %s size %zi ptr %p",
...
sdcclResult_t sdcclShmIpcClose(sdcclShmIpcDesc_t *desc) {
  if (desc) {
    SDCCLCHECK(sdcclShmClose(desc->handle));
  }
  return sdcclSuccess;
}
