...
void initEnv() {
  ...
  if (userDir) {
    sprintf(confFilePath, "%s/.sdccl.conf", userDir);
    setEnvFile(confFilePath);
  }
  sprintf(confFilePath, "/etc/sdccl.conf");
  setEnvFile(confFilePath);
}
...
void sdcclLoadParam(char const *env, int64_t deftVal, int64_t uninitialized,
  ...
  if (__atomic_load_n(cache, __ATOMIC_RELAXED) == uninitialized) {
    const char *str = sdcclGetEnv(env);
    ...
    if (str && strlen(str) > 0) {
      ...
      if (errno) {
        value = deftVal;
        INFO(SDCCL_ALL, "Invalid value %s for %s, using default %lld.", str,
             env, (long long)deftVal);
      } else {
        INFO(SDCCL_ENV, "%s set by environment to %lld.", env,
             (long long)value);
      }
    }
