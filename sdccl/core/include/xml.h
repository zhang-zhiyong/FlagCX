/*************************************************************************
 * Copyright (c) 2019-2022, NVIDIA CORPORATION. All rights reserved.
 *
 * See LICENSE.txt for license information
 ************************************************************************/

#ifndef SDCCL_XML_H_
#define SDCCL_XML_H_

#include "core.h"

#define MAX_STR_LEN 255
#define MAX_ATTR_COUNT 16
#define MAX_SUBS 128

#define NODE_TYPE_NONE 0
#define NODE_TYPE_OPEN 1
#define NODE_TYPE_CLOSE 2
#define NODE_TYPE_SINGLE 3

struct sdcclXmlNode {
  char name[MAX_STR_LEN + 1];
  struct {
    char key[MAX_STR_LEN + 1];
    char value[MAX_STR_LEN + 1];
  } attrs[MAX_ATTR_COUNT + 1]; // Need an extra one to consume extra params
  int nAttrs;
  int type;
  struct sdcclXmlNode *parent;
  struct sdcclXmlNode *subs[MAX_SUBS];
  int nSubs;
};

struct sdcclXml {
  int maxIndex, maxNodes;
  struct sdcclXmlNode nodes[1];
};

#define SDCCL_TOPO_XML_VERSION 1

// read a xml file and convert it into a sdcclXml structure
// TODO: implement the function
sdcclResult_t sdcclTopoGetXmlFromFile(const char *xmlTopoFile,
                                        struct sdcclXml *xml, int warn);
// convert a sdcclXml structure into a xml file
// TODO: implement the function
sdcclResult_t sdcclTopoDumpXmlToFile(const char *xmlTopoFile,
                                       struct sdcclXml *xml);

// auto-detect functions
// TODO: implement the following 2 functions
sdcclResult_t sdcclTopoFillApu(struct sdcclXml *xml, const char *busId,
                                 struct sdcclXmlNode **gpuNode);
sdcclResult_t sdcclTopoFillNet(struct sdcclXml *xml, const char *pciPath,
                                 const char *netName,
                                 struct sdcclXmlNode **netNode);

sdcclResult_t xmlGetApuByIndex(struct sdcclXml *xml, int apu,
                                struct sdcclXmlNode **apuNode);
sdcclResult_t xmlFindClosestNetUnderCpu(struct sdcclXml *xml,
                                         struct sdcclXmlNode *apuNode,
                                         struct sdcclXmlNode **retNet);
sdcclResult_t xmlFindClosestNetUnderServer(struct sdcclXml *xml,
                                            struct sdcclXmlNode *apuNode,
                                            struct sdcclXmlNode **retNet);

static size_t xmlMemSize(int maxNodes) {
  return offsetof(struct sdcclXml, nodes) +
         sizeof(struct sdcclXmlNode) * maxNodes;
}
static sdcclResult_t xmlAlloc(struct sdcclXml **xml, int maxNodes) {
  char *mem;
  SDCCLCHECK(sdcclCalloc(&mem, xmlMemSize(maxNodes)));
  *xml = (struct sdcclXml *)mem;
  (*xml)->maxNodes = maxNodes;
  return sdcclSuccess;
}

static sdcclResult_t xmlGetAttrIndex(struct sdcclXmlNode *node,
                                      const char *attrName, int *index) {
  *index = -1;
  const int nAttrs = node->nAttrs;
  for (int a = 0; a < nAttrs; a++) {
    if (strncmp(node->attrs[a].key, attrName, MAX_STR_LEN) == 0) {
      *index = a;
      return sdcclSuccess;
    }
  }
  return sdcclSuccess;
}

static sdcclResult_t xmlGetAttr(struct sdcclXmlNode *node,
                                 const char *attrName, const char **value) {
  int index;
  SDCCLCHECK(xmlGetAttrIndex(node, attrName, &index));
  *value = index == -1 ? NULL : node->attrs[index].value;
  return sdcclSuccess;
}

static sdcclResult_t xmlGetAttrStr(struct sdcclXmlNode *node,
                                    const char *attrName, const char **value) {
  SDCCLCHECK(xmlGetAttr(node, attrName, value));
  if (*value == NULL) {
    WARN("Attribute %s of node %s not found", attrName, node->name);
    return sdcclInternalError;
  }
  return sdcclSuccess;
}
static sdcclResult_t xmlGetAttrInt(struct sdcclXmlNode *node,
                                    const char *attrName, int *value) {
  const char *str;
  SDCCLCHECK(xmlGetAttrStr(node, attrName, &str));
  *value = strtol(str, NULL, 0);
  return sdcclSuccess;
}

static sdcclResult_t xmlGetAttrIntDefault(struct sdcclXmlNode *node,
                                           const char *attrName, int *value,
                                           int defaultValue) {
  const char *str;
  SDCCLCHECK(xmlGetAttr(node, attrName, &str));
  *value = str ? strtol(str, NULL, 0) : defaultValue;
  return sdcclSuccess;
}

static sdcclResult_t xmlGetAttrLong(struct sdcclXmlNode *node,
                                     const char *attrName, int64_t *value) {
  const char *str;
  SDCCLCHECK(xmlGetAttrStr(node, attrName, &str));
  *value = strtol(str, NULL, 0);
  return sdcclSuccess;
}

static sdcclResult_t xmlGetAttrFloat(struct sdcclXmlNode *node,
                                      const char *attrName, float *value) {
  const char *str;
  SDCCLCHECK(xmlGetAttrStr(node, attrName, &str));
  *value = strtof(str, NULL);
  return sdcclSuccess;
}

static sdcclResult_t xmlFindTag(struct sdcclXml *xml, const char *tagName,
                                 struct sdcclXmlNode **node) {
  *node = NULL;
  for (int i = 0; i < xml->maxIndex; i++) {
    struct sdcclXmlNode *n = xml->nodes + i;
    if (strcmp(n->name, tagName) == 0) {
      *node = n;
      return sdcclSuccess;
    }
  }
  return sdcclSuccess;
}

static sdcclResult_t xmlFindNextTag(struct sdcclXml *xml, const char *tagName,
                                     struct sdcclXmlNode *prev,
                                     struct sdcclXmlNode **node) {
  *node = NULL;
  for (int i = prev - xml->nodes + 1; i < xml->maxIndex; i++) {
    struct sdcclXmlNode *n = xml->nodes + i;
    if (strcmp(n->name, tagName) == 0) {
      *node = n;
      return sdcclSuccess;
    }
  }
  return sdcclSuccess;
}

static sdcclResult_t xmlFindTagKv(struct sdcclXml *xml, const char *tagName,
                                   struct sdcclXmlNode **node,
                                   const char *attrName,
                                   const char *attrValue) {
  *node = NULL;
  for (int i = 0; i < xml->maxIndex; i++) {
    struct sdcclXmlNode *n = xml->nodes + i;
    if (strcmp(n->name, tagName) == 0) {
      const char *value;
      SDCCLCHECK(xmlGetAttr(n, attrName, &value));
      if (value && strcmp(value, attrValue) == 0) {
        *node = n;
        return sdcclSuccess;
      }
    }
  }
  return sdcclSuccess;
}

static sdcclResult_t xmlSetAttr(struct sdcclXmlNode *node,
                                 const char *attrName, const char *value) {
  int index;
  SDCCLCHECK(xmlGetAttrIndex(node, attrName, &index));
  if (index == -1) {
    index = node->nAttrs++;
    strncpy(node->attrs[index].key, attrName, MAX_STR_LEN);
    node->attrs[index].key[MAX_STR_LEN] = '\0';
  }
  strncpy(node->attrs[index].value, value, MAX_STR_LEN);
  node->attrs[index].value[MAX_STR_LEN] = '\0';
  return sdcclSuccess;
}

static sdcclResult_t xmlSetAttrIfUnset(struct sdcclXmlNode *node,
                                        const char *attrName,
                                        const char *value) {
  int index;
  SDCCLCHECK(xmlGetAttrIndex(node, attrName, &index));
  if (index != -1)
    return sdcclSuccess;
  index = node->nAttrs++;
  strncpy(node->attrs[index].key, attrName, MAX_STR_LEN);
  node->attrs[index].key[MAX_STR_LEN] = '\0';
  strncpy(node->attrs[index].value, value, MAX_STR_LEN);
  node->attrs[index].value[MAX_STR_LEN] = '\0';
  return sdcclSuccess;
}

static sdcclResult_t xmlSetAttrInt(struct sdcclXmlNode *node,
                                    const char *attrName, const int value) {
  int index;
  SDCCLCHECK(xmlGetAttrIndex(node, attrName, &index));
  if (index == -1) {
    index = node->nAttrs++;
    strncpy(node->attrs[index].key, attrName, MAX_STR_LEN);
    node->attrs[index].key[MAX_STR_LEN] = '\0';
  }
  snprintf(node->attrs[index].value, MAX_STR_LEN, "%d", value);
  node->attrs[index].value[MAX_STR_LEN] = '\0';
  return sdcclSuccess;
}

static sdcclResult_t xmlSetAttrFloat(struct sdcclXmlNode *node,
                                      const char *attrName, const float value) {
  int index;
  SDCCLCHECK(xmlGetAttrIndex(node, attrName, &index));
  if (index == -1) {
    index = node->nAttrs++;
    strncpy(node->attrs[index].key, attrName, MAX_STR_LEN);
    node->attrs[index].key[MAX_STR_LEN] = '\0';
  }
  snprintf(node->attrs[index].value, MAX_STR_LEN, "%g", value);
  node->attrs[index].value[MAX_STR_LEN] = '\0';
  return sdcclSuccess;
}

static sdcclResult_t xmlSetAttrLong(struct sdcclXmlNode *node,
                                     const char *attrName,
                                     const int64_t value) {
  int index;
  SDCCLCHECK(xmlGetAttrIndex(node, attrName, &index));
  if (index == -1) {
    index = node->nAttrs++;
    strncpy(node->attrs[index].key, attrName, MAX_STR_LEN);
    node->attrs[index].key[MAX_STR_LEN] = '\0';
  }
  snprintf(node->attrs[index].value, MAX_STR_LEN, "%#lx", value);
  node->attrs[index].value[MAX_STR_LEN] = '\0';
  return sdcclSuccess;
}

static sdcclResult_t xmlInitAttrUint64(struct sdcclXmlNode *node,
                                        const char *attrName,
                                        const uint64_t value) {
  int index;
  SDCCLCHECK(xmlGetAttrIndex(node, attrName, &index));
  if (index == -1) {
    index = node->nAttrs++;
    strncpy(node->attrs[index].key, attrName, MAX_STR_LEN);
    snprintf(node->attrs[index].value, MAX_STR_LEN, "0x%lx", value);
  }
  return sdcclSuccess;
}

static sdcclResult_t xmlUnsetAttr(struct sdcclXmlNode *node,
                                   const char *attrName) {
  int index;
  SDCCLCHECK(xmlGetAttrIndex(node, attrName, &index));
  if (index == -1)
    return sdcclSuccess;
  for (int i = index + 1; i < node->nAttrs; i++) {
    strcpy(node->attrs[i - 1].key, node->attrs[i].key);
    strcpy(node->attrs[i - 1].value, node->attrs[i].value);
  }
  node->nAttrs--;
  return sdcclSuccess;
}

static sdcclResult_t xmlGetSub(struct sdcclXmlNode *node, const char *subName,
                                struct sdcclXmlNode **sub) {
  *sub = NULL;
  for (int s = 0; s < node->nSubs; s++) {
    if (strcmp(node->subs[s]->name, subName) == 0) {
      *sub = node->subs[s];
      return sdcclSuccess;
    }
  }
  return sdcclSuccess;
}

static sdcclResult_t xmlGetSubKv(struct sdcclXmlNode *node,
                                  const char *subName,
                                  struct sdcclXmlNode **sub,
                                  const char *attrName, const char *attrValue) {
  *sub = NULL;
  for (int s = 0; s < node->nSubs; s++) {
    struct sdcclXmlNode *subNode = node->subs[s];
    if (strcmp(subNode->name, subName) == 0) {
      const char *value;
      SDCCLCHECK(xmlGetAttr(subNode, attrName, &value));
      if (value && strcmp(value, attrValue) == 0) {
        *sub = node->subs[s];
        return sdcclSuccess;
      }
    }
  }
  return sdcclSuccess;
}
static sdcclResult_t xmlGetSubKvInt(struct sdcclXmlNode *node,
                                     const char *subName,
                                     struct sdcclXmlNode **sub,
                                     const char *attrName,
                                     const int attrValue) {
  char strValue[10];
  snprintf(strValue, 10, "%d", attrValue);
  SDCCLCHECK(xmlGetSubKv(node, subName, sub, attrName, strValue));
  return sdcclSuccess;
}

static sdcclResult_t xmlAddNode(struct sdcclXml *xml,
                                 struct sdcclXmlNode *parent,
                                 const char *subName,
                                 struct sdcclXmlNode **sub) {
  if (xml->maxIndex == xml->maxNodes) {
    WARN("Error : too many XML nodes (max %d)", xml->maxNodes);
    return sdcclInternalError;
  }
  struct sdcclXmlNode *s = xml->nodes + xml->maxIndex++;
  s->nSubs = 0;
  s->nAttrs = 0;
  *sub = s;
  s->parent = parent;
  if (parent) {
    if (parent->nSubs == MAX_SUBS) {
      WARN("Error : too many XML subnodes (max %d)", MAX_SUBS);
      return sdcclInternalError;
    }
    parent->subs[parent->nSubs++] = s;
  }
  strncpy(s->name, subName, MAX_STR_LEN);
  s->name[MAX_STR_LEN] = '\0';
  return sdcclSuccess;
}

static sdcclResult_t xmlRemoveNode(struct sdcclXmlNode *node) {
  node->type = NODE_TYPE_NONE;
  struct sdcclXmlNode *parent = node->parent;
  if (parent == NULL)
    return sdcclSuccess;
  int shift = 0;
  for (int s = 0; s < parent->nSubs; s++) {
    if (parent->subs[s] == node)
      shift = 1;
    else if (shift)
      parent->subs[s - 1] = parent->subs[s];
  }
  parent->nSubs--;
  return sdcclSuccess;
}

// Dictionary for STR -> INT conversions. No dictionary size information,
// there needs to be a last element with str == NULL.
struct kvDict {
  const char *str;
  int value;
};

static sdcclResult_t kvConvertToInt(const char *str, int *value,
                                     struct kvDict *dict) {
  struct kvDict *d = dict;
  while (d->str) {
    if (strncmp(str, d->str, strlen(d->str)) == 0) {
      *value = d->value;
      return sdcclSuccess;
    }
    d++;
  }
  INFO(SDCCL_GRAPH,
       "KV Convert to int : could not find value of '%s' in dictionary, "
       "falling back to %d",
       str, d->value);
  *value = d->value;
  return sdcclSuccess;
}
static sdcclResult_t kvConvertToStr(int value, const char **str,
                                     struct kvDict *dict) {
  struct kvDict *d = dict;
  while (d->str) {
    if (value == d->value) {
      *str = d->str;
      return sdcclSuccess;
    }
    d++;
  }
  WARN("KV Convert to str : could not find value %d in dictionary", value);
  return sdcclInternalError;
}

#endif // SDCCL_XML_H_
