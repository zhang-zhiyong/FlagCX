...
sdcclResult_t mcclAdaptorCommMuDevice(const sdcclInnerComm_t comm,
                                       int *device) {
  return (sdcclResult_t)mcclCommMuDevice(comm->base, device);
}
...
struct sdcclCCLAdaptor musa_mcclAdaptor = {
    "MCCL",
    // Basic functions
